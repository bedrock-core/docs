---
sidebar_position: 1
description: "Themed modal form."
---
# Form

Themed modal form. Wraps the [`Form`](/docs/ui/components/Form) primitive with authentic Minecraft textures from the [theme](../theme.md) — same config (`onSubmit`/`onCancel`), a themed field for each runtime field, and a couple of ore-styled-only conveniences.
...
- [**`Form`**](/docs/ui/components/Form#props) — the root component; unchanged from the primitive.
...
Unchanged from the primitive — see the runtime [`Form`'s Rules & Restrictions](/docs/ui/components/Form#rules): exactly one `Form.Button type="submit"` required, at most one `type="exit"`, no nesting, no plain `Button` inside.
...
- Prefer this themed `Form` for any screen that should match the rest of your Minecraft-styled UI; drop to the raw runtime [`Form`](/docs/ui/components/Form) only when building fully custom, unstyled UI.
