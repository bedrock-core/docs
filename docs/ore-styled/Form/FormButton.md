...
Wraps the runtime [`Form.Button`](/docs/ui/components/Form/FormButton) primitive with the [theme](../theme.md)'s textures. The label rides the modal's title payload as plain text, so only the variant's **color** reaches it — its font and scale do not.
