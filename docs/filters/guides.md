---
sidebar_position: 5
description: "Compiles MDX guides into a guide manifest and auto-localized .lang entries."
---
# guides

Compiles **MDX guide content** into the two build artifacts [`@bedrock-core/guides`](/docs/guides) consumes:
...
`guides` writes its `.lang` entries **before** the [i18n filter](./i18n.md) carries them into its bundle's passthrough, which is how guide keys reach the runtime's text-measurement source. The bundler then inlines both generated modules.
:::
...
| Rules | `---` |
| MDX components | `<Name prop="x" n={1} flag />` — literal props only; rendered through the [component registry](/docs/guides#custom-components) |
...
Admonition **default** titles (Note, Tip, …) are not this filter's output. They are `@bedrock-core/guides`' own typed i18n resources under `core.guides.adm.*`, folded into your bundle by the [i18n filter's library discovery](./i18n.md#libraries) — override them there.
...
The derived value is the same `<creator>_<pack>` join [the i18n filter derives](./i18n.md#namespacing) from `core.register({ manifest: { creator, pack } })` and the server runtime builds at startup, so every key your pack emits sits under one prefix.
