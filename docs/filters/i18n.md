...
The build half of the localization system. Nested TypeScript objects are the source of truth for your addon's text, and the filter generates everything downstream: the `.lang` files Bedrock resolves on each player's client, the runtime bundle [`@bedrock-core/i18n`](/docs/i18n) resolves on the server, and the types that make every key and interpolation variable autocomplete.
