...
| --- | --- | --- |
| [`<Form>`](../roots/Form.md) | a native field the engine owns | nothing until submit, then `values[name]` |
| [`<Screen>`](../roots/Screen.md) | a button that flips its state and renders the screen again | `onChange` on the press; the player sees the new state on the next present |
| [`<Container>`](../roots/Container.md) | a cell of the entity's container — the press is an item taken and put straight back | `onChange` on the press; the slots settle in the same tick |
...
The cell takes one index in the entity's container. A press is an item taken and put straight back, so the handler runs and the slots settle in the same tick.
