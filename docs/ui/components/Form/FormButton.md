---
sidebar_position: 8
description: "The submit/exit action button of a Form."
---
# Form.Button
...
A `Form` enforces the cardinality of these buttons at build time (see [`Form`'s Rules & Restrictions](./Form.md#rules)): exactly one `type="submit"` is required, and at most one `type="exit"` is allowed.
...
- For themed screens, prefer [`@bedrock-core/ore-styled`](/docs/ore-styled/Form/FormButton)'s `Form.Button`, which adds a `variant` prop.
