...
| `placeholder` | `string` | — | Shown on the button face when the value is empty, and inside the modal text field |
| `face` | `JSX.Node` | a `Text` showing the value (or `placeholder` when empty) | Overrides the content drawn on the button face. Lets a styled wrapper render its own face (e.g. colored value text) while reusing this component's modal + state logic. This is how [`@bedrock-core/ore-styled`](/docs/ore-styled/Input) builds its themed `Input` |
