...
| --- | --- | --- | --- |
| `name` | `string` | — | What addon logic calls this cell: `container.getItem('output')` reaches it instead of a counted index. Own slots only, and unique within a screen — see [Naming a cell](#naming-a-cell) |
| `role` | `'both' \| 'input' \| 'output'` | `'both'` | What an interactive slot lets the player do; see [Roles](#roles) |
| `interactive` | `boolean` | `true` | Whether the player can move items through the slot at all. `false` draws an inert cell — the item shows, but no take, place or drop reaches it, and `role` does not apply |
| `onInsert` | `(event: SlotEvent) => void` | — | Runs after an item arrives. `event.stack` is what landed, `event.player` who put it there, `event.host` the entity or block that owns the screen, `event.container` its own cells |
| `onRemove` | `(event: SlotEvent) => void` | — | Runs after the slot empties. `event.stack` is what left, `event.player` who took it, `event.host` the entity or block that owns the screen, `event.container` its own cells |
...
A screen's own cells are reached through `event.container` inside the component and through `screen.container(host)` outside it — the engine's own [`Container`](https://learn.microsoft.com/minecraft/creator/scriptapi/minecraft/server/container), addressed by index or by name; see [Reaching the cells](../guides/container-screens.md#reaching-the-cells). A `name` is how one particular cell is picked out of them:

```tsx
{/* The nine inputs are interchangeable, so none of them is named. */}
{Array.from({ length: 9 }, (_, index) => <Slot key={index} />)}

{/* The result is the one cell the recipe addresses. */}
<Slot name={'output'} role={'output'} />
```

```ts
for (let slot = 0; slot < container.size; slot += 1) {
  if (container.getSlot(slot).name === undefined) {
    // One of the nine inputs.
  }
}

if (container.getItem('output') === undefined) {
  container.setItem('output', new ItemStack('minecraft:crafting_table', 1));
}
```

Most cells want no name. An unnamed cell is found by walking the container's indices, which is what a grid of interchangeable inputs wants; a name is for the cell whose job is different from its neighbours'. Every name a screen declares is in `container.names`, mapped to its index.

Two things are build errors, with a message naming the fix:

- `name` on a **foreign** slot. A name reaches a cell of the container the screen owns, and a foreign slot draws someone else's collection.
- Two own slots sharing a name. A name reaches one cell, so it has to be unique within a screen — two screens may use the same name freely.

Buttons are not nameable: a press is delivered to its own handler, so there is nothing to look up.
...
Every slot costs one index in the host's container; the build sizes it to fit them all.
