...
| --- | --- | --- | --- |
| `children` | [`DisplayText`](/docs/i18n/api#displaytext) (`string \| RawMessage`) | — | The text to display. A single child only — arrays throw. Strings are auto-detected as keys or literals; `RawMessage` values are always localized. See [One text channel](#one-text-channel) |
...
:::tip Which verb?
`key()` and `raw()` keep resolution on the client, so each player reads the text in their own language. `t()` resolves server-side in the player's currently resolved locale — reach for it when you need to compose the string yourself (concatenating formatting codes, building a template). Full write-up in [The three verbs](/docs/i18n/api#the-three-verbs).
:::
...
function Title(): JSX.Element {
  const composed = useComposed((resolve, width) => {
    ...
    return width === undefined || measureText({ text: long }).width <= width
      ? long
      : resolve('drav0011_shop.title.short') ?? '';
  });
...
The build composes the title once for every language the pack ships, at the width the panel is laid out at, and the player reads their own. See [`useComposed`](../hooks/useComposed.md).
...
`Text` takes a **single** child of type [`DisplayText`](/docs/i18n/api#displaytext) — `string | RawMessage`. Literal text and localized text ride the same channel, and which one you get is detected rather than declared.
...
Inside components, bind [the typed verbs](/docs/i18n/api#the-three-verbs) with [`useTranslation(i18n)`](../hooks/useTranslation.md); outside them, use `i18n.forPlayer(player)` / `i18n.forLocale(locale)`.
