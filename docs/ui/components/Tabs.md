...
| `tabBackground` | `string` | `'textures/ui/unstyled'` | The face every header is drawn on while its tab is not chosen |
| `tabHover` | `string` | `tabBackground` | The face while the pointer is over a tab that is not chosen |
| `tabSelected` | `string` | `tabBackground` | The face while its tab is the chosen one |
...
Both inherit [control props](./control-props.md). A header is drawn on its face: the face fills the tab, and `header` or `headerSelected` sits on it. [`@bedrock-core/ore-styled`](/docs/ore-styled/Tabs)'s `Tabs` sets all three faces from the theme and takes a label instead of a drawn header.
