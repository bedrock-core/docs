...
A compiled screen's layout lives in the pack of the addon that built it, and a host realm shows it by title. Some of what a host draws is not the host's to bake: the page for one addon in the catalog — its thumbnail, its description, its authors — belongs to that addon, baked in that addon's pack, and the host has nothing to say about it but where it goes.
...
| `values`<Req /> | `readonly DisplayText[]` | — | What each slot carries this present — the marker first, then the embedded screen's values |
