...
| `params` | `Record<string, unknown>` | — | Props the target screen is rendered with; they fill what the layout already reserved. On a static screen, plain data only: strings, finite numbers, booleans, null, and arrays and objects of those |
...
A container screen has no key, so `<Link to>` cannot name one. A chest is entered by walking up to an entity, or interacting with a placed block.
