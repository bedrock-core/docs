...
A real container cell inside a [container screen](../../container-screens/container-screens.md): the player moves items through it, and the screen sees what arrives.
...
- Default: `'both'`
- Description: What an interactive slot lets the player do.
...
#### `collection`
- Type: `string`
- Description: A JSON UI collection to read instead of the screen's own — `inventory_items`, `hotbar_items`, or any collection the engine exposes. Given one, the slot is **foreign**: it draws `collection[index]`, takes no slot of the screen's own container, and the runtime never polls it. See [Own versus foreign](#own-versus-foreign) below.

#### `index`
- Type: `number`
- Description: The cell within `collection` to draw. Required, and only legal, alongside `collection`; an integer `>= 0`.
