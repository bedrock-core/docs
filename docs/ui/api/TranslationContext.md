---
sidebar_position: 4
description: "The context carrying how localized text resolves for a component subtree."
---
...
A host that renders other addons' strings needs a resolver over every published bundle. That is what [`@bedrock-core/config`](/docs/config) does — it renders every registered addon's display fields, so it provides the world resolver:
...
| Components that build display strings themselves ([`Header`](/docs/ore-styled/Header), [`MenuRow`](/docs/ore-styled/MenuRow)) | |
