...
| --- | --- | --- |
| `effect` | `() => (() => void) \| void` | Function that contains the side effect logic. Can optionally return a cleanup function |
| `deps` | `any[]` | Dependency array. Effect runs when dependencies change. Omit for every execution, empty array `[]` for mount only |
