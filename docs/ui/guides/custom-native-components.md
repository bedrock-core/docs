...
emit). The render pack's own components in `packs/RP/ui/core-ui/` are the reference implementation.
:::
...
described in [Caveats](#notes). A custom modal control only needs `nativeArgs` if
