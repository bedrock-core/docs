...
The string-keyed `core.state` (`set` / `get` / `delete` / `getNamespace` / `subscribe` on this addon's namespace, framework keys hidden) is deprecated in favour of the tree and goes away in a later minor. Everything it did is a leaf now:

```ts
// before
core.state.set('currency', 'gold');
core.node.state.get('drav0011_economy', 'currency');
core.state.subscribe(change => save(change.key, change.value));

// after
shared.currency.set('gold');
core.shared.of<EconomyShared>('drav0011_economy')?.currency.get();
// nothing — `persisted()` in the declaration
```
