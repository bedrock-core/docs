...
The template scaffolds the **whole bedrock-core stack**, not just a UI: [server runtime](/docs/server/api/runtime) registration, the shared config UI, typed translations, MDX guides, JSON generation from TypeScript, and a themed example screen with navigation and a modal form.
...
| --- | --- | --- |
| 1 | [`generator`](/docs/filters/generator) | Transpiles `.ts` templates in `BP/`/`RP/` into JSON (blocks, entities), and generates Minecraft document types |
| 2 | [`guides`](/docs/filters/guides) | Compiles `data/guides/<locale>/**.mdx` into a manifest + `.lang` |
| 3 | [`i18n`](/docs/filters/i18n) | Compiles `data/i18n/<locale>.ts` into `.lang`, the runtime bundle, and types |
| 4 | [`bundler`](/docs/filters/bundler) | Bundles `BP/scripts/` into one `main.js` with esbuild |

The order matters: `guides` writes its `.lang` section before `i18n` picks it up as measurement passthrough, and `i18n` must write its bundle before the bundler inlines it. See [the i18n filter page](/docs/filters/i18n#coexisting-with-the-guides-filter).
...
Together they form the addon namespace `<creator>_<pack>` that the [i18n filter derives](/docs/filters/i18n#namespacing) for `.lang` keys and that the `guides` filter takes as its `namespace` setting.
...
Display fields are **i18n keys**, not literals — that is what lets the shared list render your addon's name in each player's own language. `core.register()` publishes them to the [server-side registry](/docs/server/api/registry); `ui(core)` mounts the [shared config UI](/docs/config) and registers the four `<ns>:…` commands.
...
:::tip Prerequisites
Node.js 20+, a package manager, and [Regolith](https://regolith-docs.readthedocs.io/en/stable) on your `PATH`. See [Installation](/docs/ui/installation).
:::

## Next steps

:::caution Pre-1.0
`@bedrock-core/cli` is under active development. Breaking changes can still land until `1.0.0` — pin exact versions and read the release notes before upgrading.
:::
...
- [config](/docs/config) — the settings UI `ui(core)` mounts
- [guides](/docs/guides) — the in-game guide the template seeds
- [ore-styled](/docs/ore-styled) — the components `Example.tsx` uses
- [server-runtime](/docs/server/api/runtime) — the `core.register()` half of the generated `main.ts`, and [how the two sides meet](/docs/server/guides/ui-integration)
