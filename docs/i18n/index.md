...
description: "@bedrock-core/i18n is the localization layer for Minecraft Bedrock addons: typed keys, typed interpolation and plurals, resolved on the client in each…"
---
...
It is the runtime half of a two-part system. The build half is the [`i18n` Regolith filter](/docs/filters/i18n), which turns your `packs/data/i18n/<locale>.ts` modules into `.lang` files, a runtime bundle and the types this package's API infers from.
...
That is the whole setup. Key paths, interpolation variables and plural forms are all inferred from the bundle's type — no module augmentation, no manual type imports. See [the filter page](/docs/filters/i18n#tsconfig) for the `@bedrock-core/generated/i18n` path alias this import needs.
...
Inside a component, bind the addon's typed verbs to the viewing player with [`useTranslation`](/docs/ui/hooks/useTranslation):
...
[`TranslationContext`](/docs/ui/api/TranslationContext) exists to **override** the default for a subtree — for hosts that resolve beyond their own bundle (`@bedrock-core/config` provides `core.translations.forPlayer(player)`, which chains every addon's published bundle) or subtrees pinned to custom data. [`useTranslationResolver`](/docs/ui/hooks/useTranslationResolver) reads whichever resolver is active.
...
- **Vanilla `t()` values are partial by design.** Only the vanilla keys your compiled scripts reference as literals or `$.vanilla.` chains ride in the bundle — see [the filter page](/docs/filters/i18n#vanilla-strings).
...
:::caution Pre-1.0
`@bedrock-core/i18n` is under active development. Breaking changes can still land until `1.0.0` — pin exact versions and read the release notes before upgrading.
:::
...
| Page | Description |
| --- | --- |
| [i18n Regolith filter](/docs/filters/i18n) | Authoring `packs/data/i18n/<locale>.ts`, generated outputs and parity checks |
