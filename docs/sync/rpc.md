---
sidebar_position: 3
description: "sync.rpc — and core.rpc, which is the same object — is request/response messaging between addons."
---
...
The framework's own methods are namespaced (`core:config.show`, `core:catalog.show`). Method names are per-node, so collisions are only possible within your own addon — but a prefix keeps framework methods and yours visibly apart.
