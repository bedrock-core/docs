---
sidebar_position: 3
description: "Run the flexbox layout engine on a node tree."
---
...
| --- | --- | --- | --- |
| `root` | `LayoutNode` | — | The root of the node tree built with [`createNode`](./createNode.md). The root node's `x` and `y` are always `0, 0` |
| `refWidth` | `number` | `320` (`CANONICAL_SCREEN.width` — Pocket screen) | Viewport width used as the root's width when no explicit `width` is set on the root node, and as the reference for percentage resolution at the root level |
| `refHeight` | `number` | `210` (`CANONICAL_SCREEN.height` — Pocket screen) | Minimum height for the root node. The root grows beyond this when its content is taller, but never collapses below it |
